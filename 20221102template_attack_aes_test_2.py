...
input_data_file_name = "lab3_3_yihuohoudezhiPart{0}.npy"
traces_file_name = "lab3_3_traces{0}.npy"
# 每个part多少条曲线
num_of_each_part = 2500

##########################
# 下面是进行模板攻击的参数 #
##########################

# 只有1个part，这个part里有2500条，拆分出建模用的和攻击用的
num_of_profiling_and_pca = 2300
num_of_attack = num_of_each_part - num_of_profiling_and_pca
# 选择多少个part进行建模
part = 1
# 建模曲线首先从哪一个part开始
...
if use_pca == True:
    # 如果使用降维，则替换PoI
    PoIs = [0, 1, 2]  # 这个PoI对应的是用PCA，pca_component= 3, 相当于每一个点都取
...
if use_pca == True:
    ...
    if pca_method[pca_method_select] == "full_traces":
        ...
        for i in trange(pca_part):
            ...
            t = t[0:num_of_profiling_and_pca]
    ...
    if pca_method[pca_method_select] == "mean_9" or pca_method[pca_method_select] == "mean_256":
        ...
        for i in trange(pca_part):
            label_temp = np.load(file_path + input_data_file_name.format(i + pca_part_start_index))
            label_temp = label_temp[0:num_of_profiling_and_pca]
            ...
            trace_temp = trace_temp[0:num_of_profiling_and_pca]
            # 遍历一个Part中所有曲线
            for index, label in enumerate(label_temp):
                pca_count[label] += 1
                # # 算出新的均值
                #         meanMatrix[label] = meanMatrix[label] + (trace - meanMatrix[label]) / (count[label])
                temp_for_pca[label_temp] = temp_for_pca[label] + (trace_temp[index] - temp_for_pca[label]) / (
                pca_count[label])
...
for i in range(part):
    # 收集一下所有的输入数据
    input_data += np.load(file_path + input_data_file_name.format(i)).tolist()
    input_data = input_data[0:num_of_profiling_and_pca]
...
for i in trange(part):
    # 读取一个part的输入数据
    input_data = np.load(file_path + input_data_file_name.format(i + profiling_part_start_index))
    ...
    traces = traces[0:num_of_profiling_and_pca]
    if use_pca == True:
...
for i in trange(attack_part):
    ...
    print("这里暂时取后200条")
    traces = traces[-num_of_attack:]
    # 降维
    if use_pca == True:
        traces = pca.transform(traces)
    input_data = np.load(file_path + input_data_file_name.format(i + attack_part_start_index)).tolist()
    input_data = [hammingWeight(label) for label in input_data]
    input_data = input_data[-num_of_attack:]
    ...
    for j in range(traces.shape[0]):
        ...
        # 如果最右边的数（猜测的密钥中分数最高的 所对应的下标） + 1 == 输入
        if P_k.argsort()[-1] == input_data[j]:  # input_data[j] 是这条曲线对应的 label
            success_num += 1
